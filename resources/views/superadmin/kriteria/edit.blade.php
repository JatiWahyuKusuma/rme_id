...
@section('content')
    <div class="card card-outline card-primary">
        ...
        <div class="card-body">
            ...
            @else
                <form method="POST" action="{{ url('/kriteria/' . $kriteria->kriteria_id) }}" class="form-horizontal">
                    @csrf
                    {!! method_field('PUT') !!}
                        ...
                        <div class="col-11">
                            <select class="form-control" id="jenis_kriteria" name="jenis_kriteria">
                                <option value="Benefit"
                                    {{ old('jenis_kriteria', $kriteria->jenis_kriteria) == 'Benefit' ? 'selected' : '' }}>
                                    Benefit</option>
                                <option value="Cost"
                                    {{ old('jenis_kriteria', $kriteria->jenis_kriteria) == 'Cost' ? 'selected' : '' }}>Cost
                                </option>
                            </select>
